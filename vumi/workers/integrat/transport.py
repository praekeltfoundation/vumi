...
class IntegratOutboundTransport(Worker):
    ...
    def startWorker(self):
        self.consumer = yield self.consume('ussd.outbound.%(transport_name)s' % self.config,
...
class IntegratInboundTransport(Worker):
    ...
    def startWorker(self):
        ...
        self.publisher = yield self.publish_to('ussd.inbound.%(transport_name)s.fallback' % self.config)
        self.web_resource = yield self.start_web_resources(
            [
                (IntegratHttpResource(self.config, self.publisher), self.config['web_path']),
