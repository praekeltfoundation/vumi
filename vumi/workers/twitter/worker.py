...
class TwitterConsumer(Consumer):
    ...
    def consume_json(self, dictionary):
        ...
        log.msg(u"%s from %s said: %s" % (user.get('name'),
...
class TwitterWorker(Worker):
    ...
    def startWorker(self):
        ...
        self.consumer = yield self.start_consumer(TwitterConsumer,
                                                  self.publisher)
...
class TwitterXMPPBridgeWorker(Worker):
    ...
    def startWorker(self):
        ...
        self.consume('twitter.inbound.%(username)s.%(terms)s' %
                     self.config['twitter'],
            ...
            if not user.is_active:
                ...
                self.publisher.publish_message(Message(
                        ...
                        message="Hi! You you'll start receiving live tweets "\
                            "from netprophet as soon as they become "\
                            "available. Type STOP to stop receiving tweets."))
    ...
    def consume_twitter_message(self, message):
        ...
        message = u"@%s (%s from %s) said: %s" % (
            user.get('screen_name'),
            user.get('name', 'someone'),
            user.get('location', 'somewhere'),
            text)
        subscribers = User.objects.filter(is_active=True,
                                          groups__name='netprophet')
        print subscribers
