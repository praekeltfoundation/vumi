...
class SMSKeywordConsumer(Consumer):
    ...
    def consume_message(self, message):
        ...
        try:
            ...
            urlcallback_set = profile.urlcallback_set.filter(
                    name='sms_received')
...
class SMSKeywordWorker(Worker):
    ...
    def startWorker(self):
        log.msg("Starting the SMSKeywordWorkers for: %s" % self.config.get(
            'OPERATOR_NUMBER'))
        transport = self.config.get('TRANSPORT_NAME', 'fallback').lower()
        for network, msisdn in sorted(self.config.get(
                                'OPERATOR_NUMBER').items()):
            if len(msisdn):
                yield self.start_consumer(dynamically_create_keyword_consumer(
                    network,
                ...
                for urlcallback in urlcallback_set:
                    try:
                        ...
                        params = [
                            ...
                            ("created_at", sent_sms.created_at.strftime(
                                VUMI_DATE_FORMAT)),
                            ("updated_at", sent_sms.updated_at.strftime(
                                VUMI_DATE_FORMAT)),
                            ("delivered_at", sent_sms.delivered_at.strftime(
                                VUMI_DATE_FORMAT)),
...
def dynamically_create_receipt_consumer(name, **kwargs):
    log.msg("Dynamically creating receipt consumer for %s with %s" % (name,
        repr(kwargs)))
...
class SMSReceiptWorker(Worker):
    ...
    @inlineCallbacks
    def startWorker(self):
        Transport.objects.all()  # weird ORM hack
            ...
            yield self.start_consumer(
                    dynamically_create_receipt_consumer(str(transport.name),
                            routing_key='sms.receipt.%s' % (
                                transport.name.lower()),
                            queue_name='sms.receipt.%s' % (
                                transport.name.lower())
                        ))
...
class SMSAckConsumer(Consumer):
    ...
    def consume_message(self, message):
        ...
        try:
            ...
            log.err()
        log.msg("Message Ack %s consumed by %s" % (
...
class SMSInboundConsumer(Consumer):
    ...
    def consume_message(self, message):
        dictionary = message.payload
        log.msg("Consuming message:", message)
        log.msg("Inbound Message %s consumed by %s" % (
            repr(dictionary), self.__class__.__name__))
...
class SMSBatchConsumer(Consumer):
    ...
    def consume_message(self, message):
        dictionary = message.payload
        log.msg("SM BATCH %s consumed by %s" % (
