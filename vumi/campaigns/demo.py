...
class DemoWorker(base.BaseWorker):
    ...
    def startWorker(self):
        ...
        self.subscribe_to_transport(transport='gsm',
                                    account=self.config.get('msisdn'),
                                    callback=self.handle_inbound_sms)

        # subscribe to the Twitter transport to receive the Tweets
        self.subscribe_to_transport(transport='twitter',
                                    account=self.config.get('twitter'),
        ...
        self.sms_publisher = yield self.publish_to_transport(
            transport='sms', account='opera')
        self.xmpp_publisher = yield self.publish_to_transport(
            transport='xmpp', account='gtalk',
            identifier=self.config.get('gtalk'))

    def handle_inbound_message(self, transport, uuid, limit):
        if self.is_subscribed(uuid):
            self.say(transport, uuid, "Thanks, africa and #mhealth related "\
                         "tweets have now stopped. Dial *140*%(msisdn)s# or "\
                         "chat to %(gtalk)s to restart." %
                     self.get_subscriber_vars(uuid))
    ...
    def publish_tweet(self, tweet):
        ...
        for subscriber in local_subscribers:
            ...
            if self.is_within_quota(subscriber):
                ...
                # notify the user
                self.say(transport, subscriber, "Tweets have stopped as "\
                             "you've reached your maximum of %(limit)s "\
                             "tweets. Dial *140*%(msisdn)s or chat to "\
                             "%(gtalk)s for another %(limit)s messages." %
                         self.get_subscriber_vars(subscriber))
