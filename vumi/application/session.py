...
class TxSessionManager(object):
    ...
    def active_sessions(self):
        ...
        sessions = []
        sessions_to_expire = []
        for user_id in (yield self.r_server.smembers(skey)):
            ukey = self.r_key('session', user_id)
            if (yield self.r_server.exists(ukey)):
        ...
        # clear empty ones
        for user_ids in sessions_to_expire:
            yield self.r_server.srem(skey, user_id)
    ...
    def schedule_session_expiry(self, user_id, timeout):
        ...
        """
        ukey = self.r_key('session', user_id)
        return self.r_server.expire(ukey, timeout)
    ...
    def clear_session(self, user_id):
        ukey = self.r_key('session', user_id)
        return self.r_server.delete(ukey)
    ...
    def save_session(self, user_id, session):
        ...
        ukey = self.r_key('session', user_id)
        for s_key, s_value in session.items():
            yield self.r_server.hset(ukey, s_key, s_value)
        skey = self.r_key('active_sessions')
        yield self.r_server.sadd(skey, user_id)
        returnValue(session)
