...
class SchedulerTestCase(TestCase):
    ...
    def assertDelivered(self, message):
        delivered_messages = [TransportUserMessage.from_json(json_message)
                                for _, json_message in self._delivery_history]
    ...
    def test_scheduling(self):
        ...
        key, bucket_key = self.scheduler.schedule(delta, msg.to_json(), now)
    ...
    def test_delivery_loop(self):
        ...
        self.scheduler.schedule(delta, msg.to_json(), now)
    ...
    def test_deliver_loop_future(self):
        ...
        for i in range(0, 3):
            ...
            delta = i * 10
            key, _ = self.scheduler.schedule(delta, msg.to_json(), now)
            scheduled_time = now + delta + self.scheduler.granularity
    ...
    def test_deliver_ancient_messages(self):
        ...
        scheduled_key, _ = self.scheduler.schedule(0, msg.to_json(), way_back)
        self.assertTrue(scheduled_key)
    ...
    def test_clear_scheduled_messages(self):
        ...
        key, bucket = self.scheduler.schedule(0, msg.to_json(), scheduled_time)
        self.assertEqual(len(self.get_pending_messages()), 1)
        self.assertEqual(set([key]),
            self.scheduler.get_all_scheduled_keys())
