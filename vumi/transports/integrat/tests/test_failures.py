...
from vumi.transports.integrat import IntegratTransport, IntegratFailureWorker


class BadIntegratResource(Resource):
    isLeaf = True
...
class IntegratFailureWorkerTestCase(unittest.TestCase):

    @inlineCallbacks
    def setUp(self):
        self.today = datetime.utcnow().date()
        self.port = 9999
        self.path = '/api/v1/sms/integrat/receive/'
        self.config = {
            'transport_name': 'integrat',
    ...
    def mk_transport_worker(self, config, broker):
        worker = get_stubbed_worker(IntegratTransport, config, broker)
    ...
    def mk_failure_worker(self, config, broker, redis):
        w = get_stubbed_worker(IntegratFailureWorker, config, broker)
    ...
    def mk_resource_worker(self, body, headers=None, code=http.OK):
        ...
        self.workers.append(w)
        w.set_resources([(self.path, BadIntegratResource,
                          (body, headers, code))])
        yield w.startWorker()
        returnValue(w)
    ...
    def test_send_sms_success(self):
        ...
        self.assert_dispatched_count(1, 'integrat.event')
        self.assert_dispatched_count(0, 'integrat.failures')
    ...
    def test_send_sms_fail(self):
        ...
        self.assert_dispatched_count(0, 'integrat.event')
        self.assert_dispatched_count(1, 'integrat.failures')
        [fmsg] = self.get_dispatched('integrat.failures')
        fmsg = from_json(fmsg.body)
        self.assertTrue(
            "IntegratTransportError: No SmsId Header" in fmsg['reason'])
