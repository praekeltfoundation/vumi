...
class SmppTransceiverTransportTestCase(SmppTransportTestCase):
    ...
    def test_mt_sms_failure(self):
        ...
        [failure] = yield self.tx_helper.wait_for_dispatched_failures()
    ...
    def test_connect_status(self):
        ...
        # disconnect
        yield self.fake_smsc.disconnect()
        yield self.tx_helper.wait_for_dispatched_statuses()
    ...
    def test_tps_throttle_status(self):
        ...
        # We can't wait for delivery here, because that requires throttling to
        # end. Instead, we allow the reactor to process any immediately pending
        # things.
        yield self.tx_helper.worker_helper.broker.wait0()
    ...
    def test_tps_throttle_reconnect_status(self):
        ...
        # We can't wait and clear above (because that would require throttling
        # to end) so we only look at the last three statuses.
        msgs = yield self.tx_helper.wait_for_dispatched_statuses()
        [msg1, msg2, msg3] = msgs[-3:]
