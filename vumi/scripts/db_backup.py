...
class MigrateDbsCmd(usage.Options):
    ...
    def run(self, cfg):
        ...
        possible_overwrites = {}
        changed, processed, duplicates, overwritten = 0, 0, 0, 0
        for data in line_iter:
            record = json.loads(data)
            new_record, rule_number = self.apply_rules(copy.deepcopy(record))
            ...
            if new_record is not None:
                ...
                # Make sure that when multiple rename rules could lead to
                # duplicate keys, the keys (and thus their records)
                # generated by the LATER rules are given precedence

                use_this_record = True
                rule_precedence = {}
                max_rule = -1
                for p in possible_overwrites.get(new_record['key']):
                    rule_precedence[p['original_key']] = p['rule_number']
        ...
        cfg.emit("  %d duplicate keys." % duplicates)
        cfg.emit("  %d records overwritten." % overwritten)
        cfg.emit("  %d records in migration." % len(records))
