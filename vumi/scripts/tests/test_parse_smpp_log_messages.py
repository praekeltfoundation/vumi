...
class ParseSMPPLogMessagesTestCase(TestCase):
    ...
    def test_parse_of_line_with_limits(self):
        sample = resource_string(__name__, 'sample-smmp-output.log')
        parser = DummyLogParser({
        ...
        self.assertEqual(json.loads(parser.emit_log[0])['content'], "CODE2")
        self.assertEqual(json.loads(parser.emit_log[1])['content'], "CODE3")
